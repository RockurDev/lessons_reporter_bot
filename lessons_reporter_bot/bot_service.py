...
class BotService:
    ...
    def show_items_list(self, data: ShowItemsListCallbackData) -> BotServiceMessage:
        if data.i_t == 'S':
            ...
            student_id_to_student_map = {}

            def get_cached_student_by_id(student_id: int) -> Student | None:
                if cached_student := student_id_to_student_map.get(student_id):
                    return cached_student
                retrieved_student = self.student_storage.get_student_by_id(student_id)
                student_id_to_student_map[student_id] = retrieved_student
            ...
            formatted_items = [
                ...
                if (student := get_cached_student_by_id(report.student_id))
            ]
            text = 'Выберите отчёт:'
            row_width = 1
